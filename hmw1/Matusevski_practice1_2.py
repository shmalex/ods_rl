...
episode_data = []
for _ in tqdm.tqdm(range(episode_n), desc='Episodes '+str(episode_n)+', trajectory_n '+str(trajectory_n)):
...
success = 0
total_exp = 1000
for i in tqdm.tqdm(range(total_exp), desc='Laplase Test '+str(total_exp)+' runs'):
    state = env.reset()
    # env.render()
    # time.sleep(1)
    taxi_row, taxi_col, passenger_location, destination = env.decode(state)
...
laplase_success = success/total_exp
print('Laplase success ', laplase_success*100,'%')
...
success = 0
total_exp = 1000
for i in tqdm.tqdm(range(total_exp), desc='Policy Test '+str(total_exp)+' runs'):
    state = env.reset()
    # env.render()
    # time.sleep(1)
    taxi_row, taxi_col, passenger_location, destination = env.decode(state)
...
print('Policy success ', (success/total_exp)*100,'%')
