...
def continue_experiment(eid, versions, layers_n, episode_n, trajectory_len, trajectory_n, q_param):
    state_dim = 8
    action_n = 4
    ...
    agent = CrossEntropyMethod(eid, state_dim, action_n, layers_n)
...
def executor(tasks_dir, experiments_dir, iscontinue=False):
    ...
    for task in tasks:
        ...
        try:
            ...
            if iscontinue:
                continue_experiment(eid, versions, layers_n, episode_n, trajectory_len, trajectory_n, q_param)
            else:
                run_experiment(eid, versions, layers_n, episode_n, trajectory_len, trajectory_n, q_param)
            exit()
