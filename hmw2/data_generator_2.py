...
def worker_get_trajectory(task_queue, out_q, env, agent, trajectory_len):
    while True:
...
def start_threads_get_trajectories(num_worker_procs, agent, trajectory_n, trajectory_len):
    ...
    for index in range(num_worker_procs):
        proc = mp.Process(target=worker_get_trajectory, args=(task_q, out_q, agent, trajectory_len, index))
        procs.append(proc)
...
if __name__ == "__main__":
    executor(TASKS_DIR, EXP_DIR, iscontinue=False)
